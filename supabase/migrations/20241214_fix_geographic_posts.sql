-- =====================================================
-- FIKSE GEOGRAFISKE INNLEGG I get_posts_for_geography()
-- =====================================================
-- Problem: Dagens query ekskluderer ALLE innlegg uten gruppe/samfunn,
-- inkludert offentlige geografiske innlegg (med municipality_id/place_id)
--
-- Løsning: INKLUDER innlegg med geografi, selv om de ikke har gruppe/samfunn
...
BEGIN
  ...
  WHERE
    -- =============================================
    -- INKLUDER:
    -- 1. Gruppe-innlegg (har created_for_group_id)
    -- 2. Samfunns-innlegg (har created_for_community_id)
    -- 3. Offentlige geografiske innlegg (har municipality_id ELLER place_id)
    -- =============================================
    -- EKSKLUDER:
    -- - Personlige innlegg (INGEN gruppe, INGEN samfunn, INGEN geografi)
    (
...
COMMENT ON FUNCTION get_posts_for_geography IS
  'Henter innlegg for geografisk enhet med EKTE bubbling oppover.
   - Inkluderer offentlige geografiske innlegg (med municipality_id/place_id)
   ...
   - Ekskluderer personlige innlegg (uten geografi OG uten gruppe/samfunn)
   - Ekskluderer lukkede/skjulte gruppe-innlegg
   - Bubbling oppover: Tromsø by → Tromsø kommune → Nord-Troms → Sápmi';

DO $$
BEGIN
  RAISE NOTICE '=== GEOGRAFISKE INNLEGG FIKSET ===';
  RAISE NOTICE 'Offentlige geografiske innlegg inkluderes nå i geografiske feeds';
  RAISE NOTICE 'Personlige innlegg (uten geografi OG uten gruppe/samfunn) ekskluderes fortsatt';
END $$;
