-- Fix RLS policies for industries table
-- Allow authenticated users to suggest new industries

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Industries are viewable by everyone" ON industries;
DROP POLICY IF EXISTS "Authenticated users can suggest industries" ON industries;
DROP POLICY IF EXISTS "Platform admins can manage industries" ON industries;

-- Enable RLS
ALTER TABLE industries ENABLE ROW LEVEL SECURITY;

-- SELECT: Everyone can see approved industries
CREATE POLICY "Industries are viewable by everyone"
  ON industries FOR SELECT
  USING (is_approved = true AND is_active = true);

-- INSERT: Authenticated users can suggest industries
CREATE POLICY "Authenticated users can suggest industries"
  ON industries FOR INSERT
  WITH CHECK (
    auth.uid() IS NOT NULL
    AND created_by = auth.uid()
    AND is_approved = false
  );

-- UPDATE: Only platform admins can update industries
CREATE POLICY "Platform admins can manage industries"
  ON industries FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM platform_admins
      WHERE user_id = auth.uid() AND is_active = true
    )
  );

-- DELETE: Only platform admins can delete industries
CREATE POLICY "Platform admins can delete industries"
  ON industries FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM platform_admins
      WHERE user_id = auth.uid() AND is_active = true
    )
  );
