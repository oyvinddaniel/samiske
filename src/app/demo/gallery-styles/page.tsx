...
export default function GalleryStylesDemo() {
  ...
  const MasonryViewer = () => {
    ...
    // Simple swipe handlers - swipe down to close
    ...
    const handleTouchEnd = (e: React.TouchEvent, onClose: () => void) => {
      ...
      const diffY = endY - startY // Positive = swipe down
      const atTop = target.dataset.atTop === 'true'

      // Horizontal swipe - change image (only in single view)
      if (viewerState === 'single' && Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 50) {
        if (diffX > 0) {
    ...
    // Mobile view
    if (isMobile) {
      // Mobile: Single image view with scrollable content
      if (viewerState === 'single') {
        return (
          <div
            className="fixed inset-0 bg-black z-50 flex flex-col"
            onTouchStart={handleTouchStart}
            onTouchEnd={(e) => handleTouchEnd(e, () => setViewerState('masonry'))}
          >
            {/* Swipe handle */}
      ...
      // Mobile: Masonry gallery view
      return (
        <div
          className="fixed inset-0 bg-black z-50 flex flex-col"
          onTouchStart={handleTouchStart}
          onTouchEnd={(e) => handleTouchEnd(e, () => setActiveViewer(null))}
        >
          {/* Swipe handle */}
          <div className="flex justify-center pt-2 pb-1">
            <div className="w-10 h-1 bg-white/30 rounded-full" />
