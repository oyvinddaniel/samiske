...
import { Globe, Building2, MapPin, Check, ChevronDown, Plus } from 'lucide-react'
...
export function GeographySelector({
  ...
  const [pendingPlaceReturn, setPendingPlaceReturn] = useState(false)
  ...
  const handleAddPlaceSuccess = async () => {
    await loadAllData()
    setShowAddPlace(false)
    setPendingPlaceReturn(false)
    setOpen(true) // Reopen selector
  }

  const handleAddMunicipalitySuccess = async () => {
    await loadAllData()
    setShowAddMunicipality(false)

    // If we came from adding a place, return to add place modal
    if (pendingPlaceReturn) {
  ...
  const handleOpenAddMunicipalityFromPlace = () => {
    setShowAddPlace(false)
    setPendingPlaceReturn(true)
  ...
  return (
    <>
        ...
        <PopoverContent className="w-[350px] p-0" align="start">
          <Command shouldFilter={false}>
            ...
            <CommandList>
              ...
              <CommandGroup>
                <CommandItem onSelect={handleOpenAddPlace} className="text-blue-600">
              ...
              {/* Show municipalities if not searching */}
      ...
      <SuggestChangeModal
        open={showAddPlace}
        onOpenChange={(open) => {
          setShowAddPlace(open)
          if (!open && !pendingPlaceReturn) {
            setOpen(true) // Reopen selector if closed
          }
        }}
        entityType="place"
        suggestionType="new_item"
        onSuccess={handleAddPlaceSuccess}
        onAddMunicipality={handleOpenAddMunicipalityFromPlace}
      />
      ...
      <SuggestChangeModal
        ...
        suggestionType="new_item"
        onSuccess={handleAddMunicipalitySuccess}
      />
    </>
  )
}
